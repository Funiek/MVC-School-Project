...
namespace KOM_P.Services
{
    public class ImageService
    {
        ...
        public static string GetImage(string name, int width, int height)
        {
            ...
            basePath = Path.Combine(basePath, @$"{name}.jpg");
            if (!File.Exists(basePath))
            {
                string originalPath = Path.Combine(Environment.CurrentDirectory, @$"wwwroot\img\Products\{name}.jpg");
                Image image = Image.FromFile(originalPath);
                Bitmap bitmap = ResizeImage(image, width, height);
                bitmap.Save(basePath);
                return $"/img/Cache/{width}x{height}/{name}.jpg";
            }
            else return $"/img/Cache/{width}x{height}/{name}.jpg";
